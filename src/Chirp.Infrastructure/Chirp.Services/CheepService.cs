...
public class CheepService : ICheepService
{
    ...
    public async Task<List<CheepDTO>> GetPublicCheeps(int pageNumber, string? searchQuery)
    {
        ...
        if (searchQuery != null)
        {
            cheeps = await _cheepRepository.ReadCheepsWithSearch(
                null, searchQuery, offset, CheepsPerPage);
        }
        else
        {
            cheeps = await _cheepRepository.ReadCheeps(
                null, offset, CheepsPerPage);
        }
    ...
    public async Task<List<CheepDTO>> GetUserTimelineCheeps(string userName, int pageNumber)
    {
        ...
        // Retrieve authors followed by the user
        List<AuthorDTO> following = await _authorService.GetFollowing(userName);

        // Build list of usernames whose cheeps should appear in the timeline
        ...
        List<CheepDTO> cheeps = await _cheepRepository.ReadCheepsFromFollowers(
            userNames, offset, CheepsPerPage);
    ...
    public async Task<List<CheepDTO>> GetSavedCheeps(string userName, int pageNumber)
    {
        ...
        List<CheepDTO> savedCheeps = await _cheepRepository.ReadSavedCheeps(
            author.AuthorId, offset, CheepsPerPage);
    ...
    public async Task SaveCheepForUser(string userName, long cheepId)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Cheep with id {cheepId} doesn't exist");
        }
