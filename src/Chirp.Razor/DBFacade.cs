...
public class DBFacade : IDBFacade<CheepViewModel>
{
    ...
    public List<CheepViewModel> ReadByName(string username)
    {
        using var connection = new SqliteConnection($"Data Source={dbPath}");
        
        connection.Open();
        using var command = connection.CreateCommand();
        command.CommandText = "SELECT user.username, message.text, message.pub_date " +
